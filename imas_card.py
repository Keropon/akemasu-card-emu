...
def CommandLookup(CommandString):
    ...
    if CommandString == "7B": return "CARD_7B(7B)"
...
def ValidCommand(CommandString):
    ...
    if CommandString == "7B": return 1
    if CommandString == "7D": return 1
    if CommandString == "D0": return 1
    if CommandString == "80": return 1
    return 0
    
def CardTranslation(ArcadeString,CardFileName):
    CARDString = ArcadeString[ArcadeString.find("02 4E 53 00 00 00 30 31 30") + 27 : ArcadeString.find("02 4E 53 00 00 00 30 31 30") + 27 + 207]
    CardCRC = 0
...
def main():
    ...
    parser.add_argument('-fp', '--cardfileP', help='Name of card file to load. Example: -fp PCardHexFile')
    parser.add_argument('-fu', '--cardfileU', help='Name of card file to load. Example: -fu UCardHexFile')
    ...
    if not args.cardfileP:
        parser.error('Please specify a Card file name! Example: -fp PCARDFILE.HEX')
        sys.exit(0)

    if not args.cardfileU:
        parser.error('Please specify a Card file name! Example: -fu UCARDFILE.HEX')
        sys.exit(0)

    CardFileName = ""

    CardFileName = args.cardfileP
        ...
        while True:
            ...
            if comport['buffer'] and (clock() - comport['last_byte']) > 100000/1E6:
                ...
                while comport['buffer']:
                    ...
                    if CurrentCommand == CARD_7B and "05" in ReadInput:
                        output =b"\x02\x06\x7B\x31\x30\x30\x03\x4F"
                        print ("Reader Emulator: Sending: CARD_WRITE_IMG Command(7B) Reply: 02 06 7B 31 30 30 03 4F")
                        comport['ser'].write(output)
                        ReadInput=""    
                        
                    if CurrentCommand == CARD_80 and "05" in ReadInput:
                        if CardFileName == args.cardfileP:
                            CardFileName = args.cardfileU
                            print("Changed the card [" + CardFileName +"]")
                        elif CardFileName == args.cardfileU:
                              CardFileName = args.cardfileP
                              print("Changed the card [" + CardFileName +"]")
